...
function loadData(): DioData {
  const raw = readFileSync(DATA_PATH, "utf-8");
  return JSON.parse(raw) as DioData;
}

function findTrilhas(query: string): Trilha[] {
...
function certId(nome: string): string {
...
function genericModules(tecnologia: string): string[] {
  return [
...
function buildModulos(trilha: Trilha): string[] {
...
server.tool(
  "trilha",
  "Find DIO study tracks by technology and return a detailed study plan. " +
    ...
    "numbered list of study modules.",
  {
    tecnologia: z
      ...
      .min(1)
      .describe(
        "Technology name to search for (e.g. 'React', 'Python', 'Machine Learning'). " +
...
server.tool(
  "desafio",
  "Generate a random DIO-style coding challenge for a given level and technology. " +
    "Returns title, description, requirements, example, mentor tip, XP reward and deadline.",
  {
    ...
    tecnologia: z
      ...
      .min(1)
      .describe(
        "Target technology (e.g. 'JavaScript', 'React', 'Python'). " +
  ...
  async ({ nivel, tecnologia }) => {
    ...
    const techKey = Object.keys(bank).find(
      (k) =>
        k.toLowerCase().includes(tecnologia.toLowerCase()) ||
...
server.tool(
  "certificado",
  "Generate a fictional DIO completion certificate in Markdown for a user who finished a study track. " +
